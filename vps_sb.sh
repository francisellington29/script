...
install_singbox() {
    ...
    *)
    ...
    uuid=$UUID
    password=$(tr </dev/urandom -dc 'A-Za-z0-9' | head -c 24)
...
change_config() {
    if [ ${check_singbox} -eq 0 ]; then
        ...
        2)
            ...
            VMESS="{ \"v\": \"2\", \"ps\": \"${isp}\", \"add\": \"www.visa.com.tw\", \"port\": \"443\", \"id\": \"${new_uuid}\", \"aid\": \"0\", \"scy\": \"none\", \"net\": \"ws\", \"type\": \"none\", \"host\": \"${argodomain}\", \"path\": \"/vmess?ed=2048\", \"tls\": \"tls\", \"sni\": \"${argodomain}\", \"alpn\": \"\", \"fp\": \"\", \"allowlnsecure\": \"flase\"}"
    ...
    if [ ${check_singbox} -eq 0 ]; then
        ...
        2)
            ...
            sub_port=$(
                ...
                if [ "$port" -eq 80 ]; then echo ""; else echo "$port"; fi
            )
            start_nginx
            (
                port=$(grep -E 'listen [0-9]+;' /etc/nginx/nginx.conf | awk '{print $2}' | sed 's/;//')
                if [ "$port" -eq 80 ]; then echo ""; else green "订阅端口：$port"; fi
            )
...
manage_argo() {
    if [ ${check_argo} -eq 2 ]; then
        ...
        3)
            clear
            if [ -f /etc/alpine-release ]; then
                grep -Fq -- '--url http://localhost:8001' /etc/init.d/argo && get_quick_tunnel && change_argo_domain || {
                    green "\n当前使用固定隧道,无需获取临时域名"
                    sleep 2
                    menu
                }
            else
                grep -q 'ExecStart=.*--url http://localhost:8001' /etc/systemd/system/argo.service && get_quick_tunnel && change_argo_domain || {
                    green "\n当前使用固定隧道,无需获取临时域名"
                    sleep 2
                    menu
                }
...
menu() {
    ...
    check_singbox=$?
    check_nginx &>/dev/null
    check_nginx=$?
    check_argo &>/dev/null
    check_argo=$?
    check_nezha &>/dev/null
    check_nezha=$?
    ...
    check_nezha_status=$(check_nezha) >/dev/null 2>&1
    clear
    echo ""
    ...
    purple "---Nezha 状态: ${check_nezha_status}\n"
    green "1. 安装sing-box"
    red "2. 卸载sing-box"
    echo "==============="
    green "3. sing-box管理"
